            ...
            <div class="flex flex-col md:flex-row items-center justify-between">
                <div class="flex flex-1 items-center">
                    ...
                    <div class="text-center md:text-left">
                        ...
                        <p class="text-blue-100">{{ ucfirst(Auth::user()->role) }}</p>
                    </div>
                </div>
            </div>
        </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div id="birthdate-display" class="text-gray-600">
                                                        @if(Auth::user()->birthdate)
                                                            {{ \Carbon\Carbon::parse(Auth::user()->birthdate)->format('M d, Y') }}
                                                        @else
                                                            Not specified
                                                        @endif
                                                    </div>
                                        ...
                                        <div id="birthdate-edit-mode" class="hidden ml-7">
                                            <form id="birthdate-update-form" class="space-y-3">
                                                <input type="date" id="birthdate-input"
                                                       ...
                                                       value="{{ Auth::user()->birthdate ?? '' }}" max="{{ date('Y-m-d') }}">
                                        ...
                                        <div id="phone-edit-mode" class="hidden ml-7">
                                            <form id="phone-update-form" class="space-y-3">
                                                <input type="tel" id="phone-input" placeholder="Enter your phone number (09XXXXXXXXX)"
                                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                       value="{{ Auth::user()->phone ?? '' }}" maxlength="11" pattern="09[0-9]{9}" required>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-gray-600">{{ Auth::user()->username ?? 'N/A' }}</div>
                                        </div>
                                    </div>
...
function cancelGenderEdit() {
    ...
    genderInput.value = '{{ Auth::user()->gender ?? '' }}';
}
...
function cancelBirthdateEdit() {
    ...
    birthdateInput.value = '{{ Auth::user()->birthdate ?? '' }}';
}
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            if (file) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const newImagePath = '{{ asset("storage/") }}/' + data.path;
                        if (profileImg) {
    ...
    if (phoneUpdateForm) {
        phoneUpdateForm.addEventListener('submit', function(e) {
            ...
            // Update phone number
            fetch('{{ route("admin.phone.update") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    if (genderUpdateForm) {
        genderUpdateForm.addEventListener('submit', function(e) {
            ...
            // Update gender
            fetch('{{ route("admin.gender.update") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    if (birthdateUpdateForm) {
        birthdateUpdateForm.addEventListener('submit', function(e) {
            ...
            // Update birthdate
            fetch('{{ route("admin.birthdate.update") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
