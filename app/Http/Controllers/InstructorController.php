...
class InstructorController extends Controller
{
    ...
    public function saveAttendance(Request $request)
    {
        ...
        try {
            ...
            $instructorId = Auth::id();
            $programIds = Schedule::where('instructor_id', $instructorId)->pluck('program_id')->unique();
    ...
    public function updateOrNumber(Request $request, $enrollmentId)
    {
        ...
        try {
            ...
            // Only increment paid_sessions if the new OR number corresponds to the current session
            $currentSession = $request->input('session_number', null);
            if ($isNewOrNumber && $currentSession !== null) {
                // Increment paid_sessions only if currentSession equals paid_sessions + 1
                if (($enrollment->paid_sessions ?? 0) + 1 == $currentSession) {
