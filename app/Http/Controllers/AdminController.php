...
class AdminController extends Controller
{
    ...
    public function reports(Request $request)
    {
        ...
        $paymentLogs = Payment::with(['student'])->latest()->paginate(10, ['*'], 'payments_page');

        // Calculate total online payments (completed payments with online method)
        $totalOnlinePayments = Payment::where('status', 'completed')
    ...
    public function getStatementOfAccount($enrollmentId)
    {
        try {
            ...
            // Get first instructor from program schedules if available
            $instructor = null;
            if ($enrollment->program && $enrollment->program->schedules && $enrollment->program->schedules->count() > 0) {
                $firstSchedule = $enrollment->program->schedules->first();
                if ($firstSchedule && $firstSchedule->instructorUser) {
                    $instructor = ['name' => $firstSchedule->instructorUser->name];
                }
            }
    ...
    public function attendance(Request $request)
    {
        ...
        $enrolledStudents = \App\Models\User::where('role', 'student')
            ...
            ->map(function($student) use ($currentSession) {
                ...
                // Get total paid sessions from enrollment (cumulative count including manual OR number increments)
    ...
    public function certificates()
    {
        ...
        // Get eligible students who have completed all sessions in their program
        ...
        // Get issued certificates to mark generated ones
        $issuedCertificates = Certificate::with('enrollment')->get()->pluck('enrollment.student_id')->toArray();

        // Pass issued certificates to view for marking generated certificates
        return view('Admin.certificate', compact('certificates', 'eligibleStudents', 'issuedCertificates'));
    }
    ...
    public function updateOrNumber(Request $request, $enrollmentId)
    {
        ...
        try {
            ...
            $isNewOrNumber = empty($enrollment->or_number) && !empty($request->input('or_number'));
